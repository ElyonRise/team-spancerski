  ...
  return (
    <div className="glass-card p-8 rounded-3xl max-w-3xl mx-auto">
      ...
      <div className="flex gap-2 mb-8 border-b border-white/10 pb-2">
        <button 
          onClick={() => setTab('foto')} 
          className={`flex-1 py-3 rounded-2xl ${tab === 'foto' ? 'bg-green-500 text-black font-bold' : 'bg-white/10'}`}
        >
          Analisar Foto
        </button>
        <button 
          onClick={() => setTab('historico')} 
          className={`flex-1 py-3 rounded-2xl ${tab === 'historico' ? 'bg-green-500 text-black font-bold' : 'bg-white/10'}`}
        >
          Histórico
        </button>
      </div>
      ...
      {tab === 'foto' && (
        <div>
          <div className="border-2 border-dashed border-green-500/30 rounded-3xl p-12 text-center">
            {preview ? (
              <img src={preview} className="mx-auto max-h-80 rounded-2xl" alt="preview" />
            ) : (
              <p className="text-gray-400">Clique para selecionar foto da refeição</p>
            )}
            <input type="file" accept="image/*" onChange={handleFoto} className="hidden" id="fotoInput" />
            <label htmlFor="fotoInput" className="cursor-pointer mt-6 block bg-green-500/10 text-green-400 py-3 px-8 rounded-2xl inline-block">
              Selecionar Foto
            </label>
          </div>
          ...
          {foto && !resultado && (
            <button 
              ...
              disabled={loading} 
              className="w-full mt-6 bg-green-500 py-4 rounded-2xl font-bold text-black"
            >
