...
const QUICK = ['Posso substituir frango por atum?', 'O que comer antes de dormir?', 'Quantos litros de agua devo beber?', 'Qual o melhor horario para comer carboidrato?']

export default function IAPage() {
  const [messages, setMessages] = useState<{ role: string; content: string }[]>([
    { role: 'assistant', content: 'Ola! Sou o NutriBot, seu assistente especializado em nutricao da Team Spancerski. Como posso te ajudar hoje?' }
  ])
  ...
  return (
    <>
      <div className="dash-header"><h1>ASSISTENTE IA</h1><p>Especialista em nutricao disponivel 24/7</p></div>
      <div className="card mb-16" style={{ background: 'rgba(0,255,65,0.04)', border: '1px solid rgba(0,255,65,0.15)' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div>
            <div style={{ fontWeight: 700, color: 'var(--green)', fontSize: '0.9rem' }}>NUTRIBOT — Especialista em Nutricao</div>
            <p className="text-muted text-sm">Tire duvidas sobre alimentacao, macros, substitutos, hidratacao e mais. Foco exclusivo em nutricao.</p>
          </div>
        </div>
      </div>
      ...
      <div className="chat-input-row">
        <input className="chat-input" value={input} onChange={e => setInput(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && send()} placeholder="Pergunte sobre nutricao..." />
