...
export default function MotivacaoPage() {
  ...
  return (
    <>
      <div className="dash-header"><h1>ZONA DE MOTIVACAO</h1><p>Mantenha sua constancia</p></div>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <button className="btn btn-primary btn-full" onClick={markToday} disabled={isDoneToday}>
            {isDoneToday ? 'Hoje ja marcado!' : 'Marcar hoje como concluido'}
          </button>
        </div>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              { label: 'Total de dias registrados', val: totalStreak },
              { label: 'Dias esta semana', val: last7.filter(d => streak.includes(d)).length },
              { label: 'Status hoje', val: isDoneToday ? 'Concluido' : 'Pendente' },
            ].map((c: any) => (
