...
export async function setupDB() {
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS dietas (
      ...
      conteudo_raw TEXT,
      protocolo JSONB,
      kcal INT, proteina INT, carboidratos INT, gorduras INT,
      created_at TIMESTAMPTZ DEFAULT NOW()
    );
    ...
    CREATE TABLE IF NOT EXISTS registros_imc (
      id SERIAL PRIMARY KEY,
      client_id INT REFERENCES users(id) ON DELETE CASCADE,
      peso NUMERIC(5,2), altura INT, imc NUMERIC(4,2), categoria VARCHAR(50),
      recorded_at TIMESTAMPTZ DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS fotos_progresso (
      id SERIAL PRIMARY KEY,
      client_id INT REFERENCES users(id) ON DELETE CASCADE,
      url TEXT NOT NULL, tipo VARCHAR(20) DEFAULT 'progress',
      descricao TEXT, data_foto DATE, uploaded_by VARCHAR(20) DEFAULT 'client',
      created_at TIMESTAMPTZ DEFAULT NOW()
    );
